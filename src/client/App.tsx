...
export default function App() {
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      <aside className="intro-panel"><p className="eyebrow">Evidence-backed Census research</p><h1>Ask a sharper question.</h1><p className="intro-copy">Turn plain English into a transparent comparison of official Census data. Every answer keeps the source, formula, and caveats in view.</p><div className="rule" /><p className="micro-label">Try a judging question</p><div className="example-list">{examples.map((example) => <button key={example} className="example" onClick={() => void ask(example)}>{example}<span>↗</span></button>)}</div></aside>
      <section className="chat-panel"><div className="chat-heading"><div><p className="eyebrow">Research workspace</p><h2>Conversation</h2></div><span className="secure-label">Deterministic tools · v0.1</span></div>
        ...
        <p className="composer-note">Enter a question in plain English · Results use approved catalog metrics</p>
      </section>
    </section>
  </main>;
}
