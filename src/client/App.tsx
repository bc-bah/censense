...
function Evidence({ answer }: { answer: CensusAnswer }) {
  const evidence = answer.evidence;
  if (!evidence) return <div className="evidence"><div className="evidence-wide"><span>Evidence</span><strong>Evidence metadata was not included in this response.</strong></div></div>;
  return <div className="evidence"><div><span>Dataset</span><strong>{evidence.dataset}</strong></div><div><span>Vintage</span><strong>{evidence.vintage}</strong></div><div><span>Geography</span><strong>{evidence.geography}</strong></div><div><span>Calculation</span><strong>{evidence.calculation}</strong></div><div className="evidence-wide"><span>Filters</span><strong>{Object.entries(evidence.filters).map(([key, value]) => `${key}: ${value}`).join(' · ')}</strong></div><div className="evidence-wide"><span>Variables</span><strong>{evidence.variables.map((variable) => `${variable.id} · ${variable.label}`).join(' / ')}</strong></div><div className="evidence-wide"><span>Raw values</span><pre>{JSON.stringify(evidence.rawValues.slice(0, 10), null, 2)}{evidence.rawValues.length > 10 ? '\n... additional rows available in the result table' : ''}</pre></div><div className="evidence-wide source-links"><span>Source requests</span>{evidence.requests.map((request) => <a key={request.vintage} href={request.url} target="_blank" rel="noreferrer">Open {request.vintage} ACS request ↗</a>)}</div></div>;
}
